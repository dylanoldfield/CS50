...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // split line into parts method, request-target, version
            
            // create string literal for line
            char lineLiteral[strlen(line)];
            const char delim[2] = " ";
            
            // populate array
            for (int i = 0, length = strlen(line); i <= length; i++)
            {
                lineLiteral[i] = *(line+i);
            }
            
            // create separate strings
            char* lineMethod = strtok(lineLiteral, delim);
            char* lineTarget = strtok(NULL, delim);
            char* lineHTTP = strtok(NULL, delim);
            
            //repeat to separate lineTarget into path and query
            char lineTmp[strlen(lineTarget)];
            const char quest[2] = "?";
            
             // populate array adding 
            for (int i = 0, length = strlen(lineTarget); i <= length; i++)
            {
                lineTmp[i] = *(lineTarget+i);
            }
            const char* linePath = strtok(lineTmp, quest);
            const char* lineQuery = strtok(NULL,quest);
            
            //set query to null character if there is no query 
            if(lineQuery == NULL)
            {
                lineQuery = "";
            }
            ...
            // check if " is present in request-target
            char lineTargetTmp[strlen(lineTarget)];
            strcpy(lineTargetTmp,lineTarget);
            if(strchr(lineTargetTmp, '\"') != NULL)
            {
               error(400);
               continue; 
            }
            // check if version is HTTP1.1
            char lineHTTPTmp[strlen(lineHTTP)];
            strcpy(lineHTTPTmp,lineHTTP);
            if(strstr(lineHTTPTmp, "HTTP/1.1") == NULL)
            {
                error(505);
                continue;
            }
            // check if absolute path has "."
            char linePathTmp[strlen(linePath)];
            strcpy(linePathTmp,linePath);
            if(strchr(linePathTmp,'.') == NULL)
            {
                error(501);
                continue;       
            }
            ...
            char query[strlen(lineQuery)];
            
            // populate query array using lineQuery from above
            for (int i = 0, length = strlen(lineQuery); i <= length; i++)
            {
                query[i] = lineQuery[i];
            } 

            // TODO: concatenate root and absolute-path
            // check is path has '?'and set length accordingly
            int pathLen = strlen(root) + strlen(linePath) + 2;
            bool questMark = 1;
            if( strchr(lineTarget, '?') == NULL)
            {
                pathLen = pathLen - 1; 
                questMark = 0;
            }
            ...
            // populate path adding back in the '?' if neccesary
            if ( questMark == 1 )
            {
                ...
                for (int i = strlen(root), j = 0; i < pathLen - 2; i++)
                {
                    path[i] = *(linePath+j);
                }
                path[pathLen - 2] = '?';
            ...
            else
            {
                ...
                for (int i = strlen(root), j = 0; i < pathLen - 1; i++)
                {
                    path[i] = *(linePath+j);
                }
                path[pathLen - 1] = '\0';
            ...
            char lineExtension[strlen(linePath)+1]; 
            strcpy(lineExtension,linePath);
            char* lineExten = strchr(lineExtension, '.');
            char extension[strlen(lineExten)];
            
            for(int i=0, length = strlen(lineExten); i <= length; i++)
            {
                extension[i] = lineExten[i+1];
            }
